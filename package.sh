#!/bin/bash
# Use this file to build all the packages about ISA-Agents
...
if [ "$PACKAGE_TYPE" = "miacme"  ]
then
    curl -L -O https://bitbucket.org/agbeltran/isaagents-downloads/downloads/"$CONFIGURATION"
else
    curl -L -O http://bitbucket.org/eamonnmag/isaagents-downloads/downloads/"$CONFIGURATION"
fi
