...
import org.isaagents.isacreator.effects.TitlePanel;
import org.isaagents.isacreator.gs.GSSaveAction;
import org.isaagents.isacreator.gui.io.exportisa.OutputISAFilesFromGUI;
import org.isaagents.isacreator.gui.menu.ISAcreatorMenu;
import org.isaagents.isacreator.gui.modeselection.Mode;
import org.isaagents.isacreator.validateconvert.ui.ENASubmissionUI;
...
import org.isaagents.isacreator.ontologiser.adaptors.InvestigationAdaptor;
import org.isaagents.isacreator.ontologiser.ui.OntologiserUI;
import org.isaagents.isacreator.ontologymanager.OntologyManager;
import org.isaagents.isacreator.plugins.MenuPluginTracker;
import org.isaagents.isacreator.plugins.OntologyPluginTracker;
import org.isaagents.isacreator.plugins.SpreadsheetPluginTracker;
import org.isaagents.isacreator.qrcode.ui.QRCodeGeneratorUI;
import org.isaagents.isacreator.settings.ISAcreatorProperties;
import org.isaagents.isacreator.settings.SettingsUtil;
import org.isaagents.isacreator.spreadsheet.IncorrectColumnOrderGUI;
import org.isaagents.isacreator.spreadsheet.Spreadsheet;
import org.isaagents.isacreator.utils.IncorrectColumnPositioning;
import org.isaagents.isacreator.utils.PropertyFileIO;
import org.isaagents.isacreator.validateconvert.ui.OperatingMode;
import org.isaagents.isacreator.validateconvert.ui.ValidateUI;
...
public class ISAcreator extends AnimatableJFrame implements WindowFocusListener {
    ...
    private Container createMenuBar() {
        ...
        contact.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    Desktop.getDesktop().browse(new URI("http://isa-agents.org/team/"));
                } catch (IOException e1) {
                    log.error("IOException - " + e1.getMessage());
                } catch (URISyntaxException e1) {
                    log.error("URISyntaxException - " + e1.getMessage());
        ...
        feedback.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    Desktop.getDesktop().browse(new URI("http://www.github.com/ISA-agents/ISAcreator/issues"));
                } catch (IOException e1) {
                    log.error("IOException - " + e1.getMessage());
                } catch (URISyntaxException e1) {
                    log.error("URISyntaxException - " + e1.getMessage());
        ...
        public void actionPerformed(ActionEvent ae) {
            switch (type) {
                case FULL_SCREEN:
                    ...
                    if (!glass.isVisible()) {
                        setResizable(true);
                        setSize(Agentkit.getDefaultAgentkit().getScreenSize());
                        repaint();
                    }
