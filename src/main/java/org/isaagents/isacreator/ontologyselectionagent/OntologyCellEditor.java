...
/**
 * OntologyCellEditor provides a way for the OntologySelectionAgent be be the editor for a JTable cell.
 *
...
public class OntologyCellEditor extends JTextField implements TableCellEditor {
    ...
    private static OntologySelectionAgent ontologyAgent;

    static {
        ontologyAgent = new OntologySelectionAgent();
        ontologyAgent.createGUI();
    }
    ...
    public OntologyCellEditor(final boolean allowsMultipleTerms, final boolean forceOntologySelection, Map<String, RecommendedOntology> recommendedSource) {
        ...
        // OntologySelectionAgent doesn't directly interact with the CellEditor. Instead, a PropertyChange event is
        // fired, and it is here where we listen to the event and deal with it.
        ontologyAgent.addPropertyChangeListener("selectedOntology",
        ...
        ontologyAgent.addPropertyChangeListener("noSelectedOntology",
                new PropertyChangeListener() {
                    public void propertyChange(PropertyChangeEvent evt) {
                        SwingUtilities.invokeLater(new Runnable() {
                            public void run() {
    ...
    private void setCellValue() {
        setText(ontologyAgent.getSelectedTerm());
    }
     ...
     * Stops editing of the cell and hides the OntologySelectionAgent.
     */
    public void cancelCellEditing() {
        fireEditingCanceled();
        ontologyAgent.setVisible(false);
    }
    ...
    public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
        ...
        ontologyAgent.setSelectedTerm(originalValue);
        setText(originalValue);
        ...
        if ((proposedX + OntologySelectionAgent.WIDTH) > desktopBounds.width) {
            int difference = (proposedX + OntologySelectionAgent.WIDTH) -
                    desktopBounds.width;
            proposedX = proposedX - difference;
        }

        if ((proposedY + OntologySelectionAgent.HEIGHT) > desktopBounds.height) {
            int difference = (proposedY + OntologySelectionAgent.HEIGHT) -
                    desktopBounds.height;
            proposedY = proposedY - difference;
        }

        // Set ontology agent parameters
        ontologyAgent.setRecommendedOntologies(recommendedOntologyMap);
        ontologyAgent.setForceOntologySelection(forceOntologySelection);
        ontologyAgent.setMultipleTermsAllowed(allowsMultipleTerms);

        ontologyAgent.updatehistory();
        ontologyAgent.addNotify();
        ontologyAgent.setLocation(proposedX, proposedY);
        ontologyAgent.makeVisible();
//        ontologyAgent.loadRecommendedOntologiesIfAllowed();
        return this;
    }
    ...
    /**
     * Stop Editing of a cell component and set the cell value to be the term(s) selected in the OntologySelectionAgent.
     *
     * @return true
     */
